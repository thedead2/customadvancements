...
public class GenerateAdvancementCommand extends ModCommand {
    private static final SuggestionProvider<CommandSource> SUGGEST_ADVANCEMENTS = (commandContext, suggestionsBuilder) -> {
        Collection<Advancement> collection = commandContext.getSource().getServer().getAdvancementManager().getAllAdvancements();
        return ISuggestionProvider.func_212476_a(collection.stream().map(Advancement::getId), suggestionsBuilder);
    };

    protected GenerateAdvancementCommand(LiteralArgumentBuilder<CommandSource> literalArgumentBuilder) {
        super(literalArgumentBuilder);
    }


    public static void register(){
        newModCommand("generate/advancement/[advancement]", ResourceLocationArgument.resourceLocation(), SUGGEST_ADVANCEMENTS, command -> {
            ...
            if (advancement.getNamespace().equals(MOD_ID)){
                command.getSource().sendErrorMessage(TranslationKeyProvider.chatMessage("generating_game_advancement_already_exists", TextFormatting.RED, advancement.toString()));
                return COMMAND_FAILURE;
            }
            try {
                AdvancementHandler.writeAdvancementToFile(ResourceLocationArgument.getAdvancement(command, "advancement"));
            } catch (IOException e) {
                command.getSource().sendErrorMessage(TranslationKeyProvider.chatMessage("generating_game_advancement_failed", advancement));
                CrashHandler.getInstance().handleException("Unable to generate file for: " + advancement, e, Level.WARN);
                return COMMAND_FAILURE;
            }
            command.getSource().sendFeedback(TranslationKeyProvider.chatMessage("generating_game_advancement_success", advancement.toString()), false);
