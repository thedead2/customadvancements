...
public class FakeAdvancementWidget extends GuiComponent {
    ...
    public static final int ICON_Y = 5;
    ...
    public final FakeAdvancementTab tab;
    ...
    public void drawHover(PoseStack pPoseStack, int pX, int pY, float pFade, int pWidth, int pHeight, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        int yPos = pY + this.y;
        int xPos = pX + this.x;
        int tooltipStartPosition = tooltipWiderThanScreen ? xPos - this.tooltipWidth + ICON_WIDTH + 6 : xPos;
        ...
        for(int i = 0; i < this.description.size(); ++i) {
            yPosDescription = tooltipLongerThanScreen ? (float)(yPos + HEIGHT - this.tooltipHeight + 7 + i * 9) : (float)(pY + this.y + TITLE_Y + 17 + i * 9);
    ...
    protected void renderTooltipBackground(PoseStack pPoseStack, int pX, int pY, int pWidth, int pHeight, int pPadding, int pUWidth, int pVHeight, int pUOffset, int pVOffset, int yPos) {
        ...
        this.blit(pPoseStack, pX, yPos, BOX_X, advancementwidgettype.getIndex() * HEIGHT, tooltipWidthDividedBy2, HEIGHT);
        this.blit(pPoseStack, pX + tooltipWidthDividedBy2, yPos, BOX_WIDTH - halfTooltipWidth, advancementwidgettype.getIndex() * FRAME_WIDTH, halfTooltipWidth, FRAME_WIDTH);
    }
    ...
    public boolean isMouseOver(int pX, int pY, int pMouseX, int pMouseY, int pWidth) {
        int leftXCorner;
        int rightXCorner;
        int topYCorner;
        int bottomYCorner;
        ...
        if (!this.tab.getScreen().isMouseOver(xPos, yPos) && !drawingTooltip) { //should be changed?
            return false;
        }
        if (drawingTooltip) {
