    ...
    public void drawHover(PoseStack pPoseStack, int pX, int pY, float pFade, int pWidth, int pHeight, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        if (flag1) {
            for(int k1 = 0; k1 < this.description.size(); ++k1) {
                yPosDescription = (float)(l + 26 - j1 + 7 + k1 * 9);
                this.minecraft.font.draw(pPoseStack, this.description.get(k1), (float)(i1 + 5), yPosDescription, -5592406);
            }
        }
        else {
            for(int l1 = 0; l1 < this.description.size(); ++l1) {
                yPosDescription = (float)(pY + this.y + 9 + 17 + l1 * 9);
                this.minecraft.font.draw(pPoseStack, this.description.get(l1), (float)(i1 + 5), yPosDescription, -5592406);
            }
        }
        this.minecraft.font.draw(pPoseStack, this.advancement.getId().toString(), (float)(i1 + 5), yPosDescription + 10F, -5592406); //renders resource location
        EditButton editButton = new EditButton((i1 + 5), (int) (yPosDescription + 20), 20, 20, Component.literal("Edit"), pButton -> {
            this.minecraft.setScreen(new ClientAdvancementGenerator(this.tab.getScreen(), this.minecraft, this.advancement));
        });
        editButton.renderButton(pPoseStack, pMouseX, pMouseY, pPartialTick);

        AddButton addButton = new AddButton((i1 + 5 + 20 + 5), (int) (yPosDescription + 20), 20, 20, Component.literal("Add"), pButton -> {
            this.minecraft.setScreen(new ClientAdvancementGenerator(this.tab.getScreen(), this.minecraft, Objects.requireNonNull(this.advancement.getParent()).getId()));
        });
        addButton.renderButton(pPoseStack, pMouseX, pMouseY, pPartialTick);
